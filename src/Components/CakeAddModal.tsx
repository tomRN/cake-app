...
const CakeAddModal = (props: CakeAddModalProps) => {
    ...
    return <Modal centered show={true} onHide={() => props.onCancel()} >
        <Form onSubmit={(e: React.SyntheticEvent) => { e.preventDefault(); handleSubmit() }}>
            ...
            <Modal.Footer>
                {props.addStatus !== "PENDING" && <Button type="submit" name="add" variant="success" onClick={(evt: any) => { }}>Save</Button>}
                {props.addStatus !== "PENDING" && <Button type="button" name="cancel" variant="secondary" onClick={(evt: any) => { props.onCancel() }}>Cancel</Button>}
            </Modal.Footer>
        </Form>
    </Modal >
